...
export default function TaskItem({ task, isCompletedToday, isSkippedToday, onComplete, onSkip }: TaskItemProps) {
  ...
  return (
    <div
      className={`rounded-xl border bg-white p-4 transition-opacity dark:bg-gray-900 ${
        isDone
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <h3
            className={`font-medium ${isDone ? "line-through text-gray-400" : "text-gray-900 dark:text-gray-100"}`}
          >
            {task.title}
          </h3>
          {task.description && (
          ...
          {task.is_recurring === 1 && (
            <span className="mt-2 inline-block rounded-full bg-indigo-50 px-2 py-0.5 text-xs text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400">
              Repetitiva
            </span>
          )}
          {isCompletedToday && (
            <span className="mt-2 ml-2 inline-block rounded-full bg-green-50 px-2 py-0.5 text-xs text-green-600 dark:bg-green-500/10 dark:text-green-400">
              Completada
            </span>
          )}
          {isSkippedToday && (
            <span className="mt-2 ml-2 inline-block rounded-full bg-yellow-50 px-2 py-0.5 text-xs text-yellow-600 dark:bg-yellow-500/10 dark:text-yellow-400">
              Omitida
            </span>
          )}
        </div>
